            ...
            <form className="space-y-6">
                ...
                <div className="bg-card-dark border border-border-dark rounded-2xl p-6">
                    ...
                    <p className="text-xs text-slate-500 mb-4">
                        Thêm các ảnh chụp thực tế của xe từ nhiều góc độ. Ảnh sẽ hiển thị trong gallery cho người dùng xem.
                    </p>
                    ...
                    {formData.gallery && formData.gallery.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                            {formData.gallery.map((url, index) => (
                                <div key={index} className="relative group aspect-video bg-background-dark rounded-lg overflow-hidden border border-border-dark">
                                    <img
                                        src={url}
                                        alt={`Gallery ${index + 1}`}
                ...
                <div className="bg-card-dark border border-border-dark rounded-2xl p-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-sm text-slate-400">{formData.exterior?.images?.length || 0} ảnh</span>
                    </div>
                    <p className="text-xs text-slate-500 mb-4">
                    ...
                    <div className="mb-4">
                        <label className="block text-sm font-medium text-slate-300 mb-2">Mô tả ngoại thất</label>
                        <textarea
                    ...
                    {formData.exterior?.images && formData.exterior.images.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {formData.exterior.images.map((url, index) => (
                                <div key={index} className="relative group aspect-video bg-background-dark rounded-lg overflow-hidden border border-border-dark">
                                    <img src={url} alt={`Ngoại thất ${index + 1}`} className="w-full h-full object-cover" />
                                    <button
                                        type="button"
                                        onClick={() => {
                                            const newImages = formData.exterior!.images.filter((_, i) => i !== index);
                                            setFormData({ ...formData, exterior: { ...formData.exterior!, images: newImages } });
                                        }}
                ...
                <div className="bg-card-dark border border-border-dark rounded-2xl p-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-sm text-slate-400">{formData.interior?.images?.length || 0} ảnh</span>
                    </div>
                    <p className="text-xs text-slate-500 mb-4">
                    ...
                    {formData.interior?.images && formData.interior.images.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {formData.interior.images.map((url, index) => (
                                <div key={index} className="relative group aspect-video bg-background-dark rounded-lg overflow-hidden border border-border-dark">
                                    <img src={url} alt={`Nội thất ${index + 1}`} className="w-full h-full object-cover" />
                                    <button
                                        type="button"
                                        onClick={() => {
                                            const newImages = formData.interior!.images.filter((_, i) => i !== index);
                                            setFormData({ ...formData, interior: { ...formData.interior!, images: newImages } });
                                        }}
                                        className="absolute top-2 right-2 p-1 bg-red-500 text-white rounded opacity-0 group-hover:opacity-100 transition-opacity"
                                    >
                                        ✕
                    ...
                    {formData.colorOptions && formData.colorOptions.length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {formData.colorOptions.map((color, index) => (
                                <div key={index} className="relative group bg-background-dark rounded-lg overflow-hidden border border-border-dark">
                                    {color.image ? (
                                        <img src={color.image} alt={color.name} className="w-full aspect-video object-cover" />
                                    ) : (
                                        <div
                ...
                <div className="bg-card-dark border border-border-dark rounded-2xl p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-300 mb-2">Tăng tốc 0-100</label>
                            <input
                                type="text"
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 mb-3">
                                Nhập mô tả dạng văn bản thuần. Xuống dòng bằng Enter, hệ thống sẽ tự động format khi hiển thị.
                            </p>
                            <textarea
