...
const AIConfigPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'promotion' | 'event' | 'review' | 'settings'>(
    'promotion',
  );
  ...
  const removeKeyword = (
    category: 'promotionStrong' | 'promotionWeak' | 'event' | 'review',
    keyword: string,
  ) => {
    if (category === 'promotionStrong') {
      setConfig({
            ...
            <div className="bg-card-dark border border-border-dark rounded-2xl p-6 space-y-6">
              <div>
                ...
                <p className="text-xs text-slate-500 mb-3">
                  Chỉ cần 1 từ khóa này xuất hiện → category = promotion
                </p>
                <div className="flex flex-wrap gap-2 mb-3">
                  {config.promotionKeywords.strong.map((kw) => (
                    ...
                    >
                      {kw}
                      <button
                        onClick={() => removeKeyword('promotionStrong', kw)}
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <button
                    onClick={() => addKeyword('promotionStrong')}
              ...
              <div>
                <h3 className="text-lg font-bold text-yellow-400 mb-2">
                  Từ khóa YẾU (cần 2+ từ = promotion)
                </h3>
                <p className="text-xs text-slate-500 mb-3">
                  Cần ít nhất 2 từ khóa này → category = promotion
                </p>
                <div className="flex flex-wrap gap-2 mb-3">
                  {config.promotionKeywords.weak.map((kw) => (
                    <span
                      key={kw}
                      className="flex items-center gap-1 px-3 py-1 bg-yellow-500/20 text-yellow-400 rounded-full text-sm"
                    >
                      {kw}
                      <button
                        onClick={() => removeKeyword('promotionWeak', kw)}
                        className="ml-1 text-red-400 hover:text-red-300"
                      >
                        ×
                      </button>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <button
                    onClick={() => addKeyword('promotionWeak')}
            ...
            <div className="bg-card-dark border border-border-dark rounded-2xl p-6">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {config.eventKeywords.map((kw) => (
                  <span
                    key={kw}
                    className="flex items-center gap-1 px-3 py-1 bg-amber-500/20 text-amber-400 rounded-full text-sm"
                  >
                    {kw}
                    <button
                      onClick={() => removeKeyword('event', kw)}
                      className="ml-1 text-red-400 hover:text-red-300"
                    >
                      ×
                    </button>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {config.reviewKeywords.map((kw) => (
                  ...
                  >
                    {kw}
                    <button
                      onClick={() => removeKeyword('review', kw)}
                      className="ml-1 text-red-400 hover:text-red-300"
                    >
                      ×
                    </button>
                ...
                {config.autoPublish.enabled && (
                  <div className="ml-8 p-4 bg-background-dark rounded-xl">
                    ...
                    <p className="text-xs text-slate-500 mt-2">
                      Chỉ tự động đăng nếu AI confidence ≥{' '}
                      {Math.round(config.autoPublish.minConfidence * 100)}%
                    </p>
                  </div>
                )}
            ...
            <div className="bg-card-dark border border-border-dark rounded-2xl p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`px-3 py-1 ${categoryColors[testResult.category]} text-white text-sm font-bold rounded-full`}
                  >
