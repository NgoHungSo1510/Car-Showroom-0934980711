                ...
                <div className="space-y-4">
                    {classificationResult && (
                        <div className="bg-card-dark border border-border-dark rounded-2xl p-6 space-y-5">
                            ...
                            <div className="bg-background-dark rounded-xl p-4">
                                ...
                                <span className={`inline-block px-4 py-2 ${categoryColors[classificationResult.category] || 'bg-slate-500'} text-white text-sm font-bold rounded-xl`}>
                            ...
                            {classificationResult.category === 'event' && (
                                <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
                                    <label className="block text-xs text-amber-400 font-bold mb-3">📅 Thông tin sự kiện</label>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs text-slate-400 mb-1">Bắt đầu</label>
                                            <p className="text-white text-sm">
                                                {classificationResult.eventStartDate
                                                    ? new Date(classificationResult.eventStartDate).toLocaleDateString('vi-VN')
                                                    : '—'}
                                            </p>
                                        </div>
                                        <div>
                            ...
                            {classificationResult.category === 'promotion' && (
                                <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4">
                                    <label className="block text-xs text-emerald-400 font-bold mb-3">🏷️ Thông tin khuyến mãi</label>
                            ...
                            {/* 5. Gắn xe chính */}
                            <div className="bg-background-dark rounded-xl p-4">
                                <label className="block text-xs text-slate-400 mb-1">Gắn xe chính (cho nút CTA)</label>
                                <p className="text-white">
                                    {classificationResult.relatedCarName
                                        ? <span className="text-primary font-medium">🚗 {classificationResult.relatedCarName}</span>
                            ...
                            {/* 6. Tóm tắt */}
                            <div className="bg-background-dark rounded-xl p-4">
                                <label className="block text-xs text-slate-400 mb-1">Tóm tắt (hiển thị trên card)</label>
                            ...
                            <div className="bg-background-dark rounded-xl p-4">
                                ...
                                <div className="space-y-2 max-h-48 overflow-y-auto">
                                    ...
                                    {(!classificationResult.contentBlocks || classificationResult.contentBlocks.length === 0) && (
                    ...
                    {importResult && (
                        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-6">
                            ...
                            <div className="space-y-2 text-sm">
                                <p><span className="text-slate-400">ID:</span> <span className="text-white font-mono text-xs">{importResult.post._id}</span></p>
                                <p><span className="text-slate-400">Tiêu đề:</span> <span className="text-white">{importResult.post.title}</span></p>
                                <p><span className="text-slate-400">Trạng thái:</span> <span className={importResult.post.status === 'published' ? 'text-emerald-400' : 'text-amber-400'}>{importResult.post.status === 'published' ? 'Đã công khai' : 'Bản nháp'}</span></p>
                            </div>
                            <a
                    ...
                    {!classificationResult && !importResult && (
                        <div className="bg-card-dark border border-border-dark rounded-2xl p-8 text-center">
                            ...
                            <p className="text-slate-400">Nhập nội dung và bấm "Test AI" để xem kết quả phân loại</p>
                            <p className="text-slate-500 text-sm mt-2">Kết quả sẽ hiển thị theo thứ tự của trang tạo bài viết</p>
                        </div>
                    )}
                </div>
            </div>
