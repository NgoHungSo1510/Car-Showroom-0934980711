...
function detectCategory(content: string): 'news' | 'review' | 'promotion' | 'event' {
    ...
    const promotionKeywords = [
        'khuyến mãi', 'khuyến mại', 'giảm giá', 'sale', 'giảm ngay',
        'ưu đãi đặc biệt', 'ưu đãi lớn', 'tặng quà', 'quà tặng',
        'giá sốc', 'giá tốt nhất', 'tiết kiệm', 'miễn phí 100%'
    ];
 ...
 */
export async function classifyContent(content: string, images?: string[]): Promise<ClassificationResult> {
    try {
  ...
  "contentBlocks": [
    ...
    {"type": "text", "content": "Đoạn 2..."}
        ...
        if (!parsed.contentBlocks || parsed.contentBlocks.length === 0) {
            ...
            parsed.contentBlocks = parsed.contentBlocks.map(block => {
        ...
        // Extract car name
        const carMatch = content.match(/VinFast\s+\w+|Toyota\s+\w+|Honda\s+\w+|Hyundai\s+\w+|Mazda\s+\w+/i);
        ...
        return {
            category,
            title: cleanContent.replace(/[🎁📅✅💰🎉]/g, '').substring(0, 80).trim(),
            excerpt: cleanContent.replace(/[🎁📅✅💰🎉]/g, '').substring(0, 200).trim(),
