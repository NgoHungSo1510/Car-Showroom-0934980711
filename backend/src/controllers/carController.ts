...
export const getAdminCars = asyncHandler(async (req: Request, res: Response) => {
  const { brand, carType, status, page = 1, limit = 20, sort = '-createdAt', search } = req.query;
...
export const update3DConfig = asyncHandler(async (req: Request, res: Response) => {
  ...
  const car = await Car.findByIdAndUpdate(id, { model3D }, { new: true, runValidators: true });
