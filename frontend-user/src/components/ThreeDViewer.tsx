...
const LoadingSpinner: React.FC = () => (
    <Html center>
        <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-white text-sm">Đang tải mô hình 3D...</p>
        </div>
    </Html>
);
...
const ThreeDViewer: React.FC<ThreeDViewerProps> = ({ config, fallbackImage, carName }) => {
    ...
    const [selectedMeshNames, setSelectedMeshNames] = useState<string[]>(
        config.colorConfigs.find((c) => c.isDefault)?.meshNames || config.colorConfigs[0]?.meshNames || []
    );
    ...
    if (!config.hasModel || !config.fileUrl) {
        return (
            <div className="relative w-full h-[400px] md:h-[500px] bg-surface rounded-2xl overflow-hidden">
                ...
                <div className="absolute bottom-4 right-4 px-3 py-1.5 bg-black/50 backdrop-blur-sm rounded-lg text-xs text-white">
    ...
    return (
        <div className="relative w-full h-[400px] md:h-[500px] bg-gradient-to-b from-surface to-background rounded-2xl overflow-hidden">
            ...
            {/* 3D Badge */}
            <div className="absolute top-4 right-4 px-3 py-1.5 bg-primary/90 rounded-lg text-xs font-bold text-white">
                3D INTERACTIVE
            </div>
        </div>
    );
