...
// Solid background colors for better visibility on light images
      ...
      <Link to={`/posts/${post.slug}`} className="block">
        ...
        <div className="relative h-48 overflow-hidden">
          {post.coverImage ? (
            ...
            <div className="w-full h-full bg-surface-hover flex items-center justify-center">
              <svg
                className="size-12 text-text-secondary opacity-50"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          {/* Category Badge */}
          <span
            className={`absolute top-3 left-3 px-2 py-1 text-xs font-bold rounded ${category.color}`}
          >
            {category.label}
          </span>
          ...
          {eventBadge && (
            <span
              className={`absolute top-3 right-3 px-2 py-1 text-xs font-bold rounded text-text-primary ${eventBadge.color}`}
            >
              {eventBadge.text}
            </span>
          )}
          {promotionBadge && (
            <span
              className={`absolute top-3 right-3 px-2 py-1 text-xs font-bold rounded text-text-primary ${promotionBadge.color}`}
            >
              {promotionBadge.text}
            </span>
          )}

          {/* Related Car Badge - always white text on dark bg */}
          {post.relatedCar && (
            <div className="absolute bottom-3 right-3 flex items-center gap-2 px-2 py-1 bg-black/80 backdrop-blur-sm rounded text-xs text-white">
        ...
        <div className="p-5">
          ...
          <div className="flex items-center justify-between text-xs text-text-secondary">
            <span>📅 {formatDate(post.publishedAt || (post as any).createdAt)}</span>
