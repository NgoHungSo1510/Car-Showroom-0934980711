...
const PostDetailPage: React.FC = () => {
    ...
    const renderContentBlock = (block: ContentBlock, index: number) => {
        switch (block.type) {
            case 'text':
                return (
                    <div key={index} className="text-text-secondary text-lg leading-relaxed whitespace-pre-wrap">
                        {block.content}
                    </div>
                );
                            ...
                            <div className="flex-1">
                                ...
                                {block.car.price && (
                                    <p className="text-sm text-text-secondary">
                                        Từ {formatPrice(block.car.price)}
                                    </p>
                                )}
                            </div>
                ...
                {isEventExpired() && (
                    <div className="max-w-4xl mx-auto mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-xl flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-red-400/70 text-sm">Sự kiện này đã kết thúc vào ngày {formatDate(post.eventEndDate)}</p>
                        </div>
                    </div>
                )}
                ...
                <header className="max-w-4xl mx-auto mb-8">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mb-4">
                        ...
                        {post.category === 'event' && post.eventEndDate && (
                            <span className={`px-3 py-1 text-sm font-medium rounded ${isEventExpired() ? 'bg-gray-500/10 text-gray-400' : 'bg-amber-500/10 text-amber-400'}`}>
