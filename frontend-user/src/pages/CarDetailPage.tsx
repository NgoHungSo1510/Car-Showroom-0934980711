        ...
        <div className="pb-16">
            ...
            <div className="container mx-auto px-4 mb-6 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    ...
                    >
                        <svg className="size-5" fill={isBookmarked ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="min-h-[400px]">
                        ...
                        <div className="space-y-8">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {car.specs.engine && (
                                        <div className="p-4 bg-surface rounded-xl">
                                            ...
                                            <p className="font-medium text-white">{car.specs.engine}</p>
                                        </div>
                                    )}
                                    {car.specs.power && (
                                        <div className="p-4 bg-surface rounded-xl">
                                            <p className="text-xs text-text-secondary mb-1">Công suất</p>
                                            <p className="font-medium text-white">{car.specs.power}</p>
                                        </div>
                                    )}
                                    {car.specs.seats && (
                                        <div className="p-4 bg-surface rounded-xl">
                                            <p className="text-xs text-text-secondary mb-1">Số chỗ</p>
                                            <p className="font-medium text-white">{car.specs.seats} chỗ</p>
                                        </div>
                                    )}
                                    {car.specs.acceleration && (
                                        <div className="p-4 bg-surface rounded-xl">
                                            <p className="text-xs text-text-secondary mb-1">Tăng tốc 0-100</p>
                                            <p className="font-medium text-white">{car.specs.acceleration}</p>
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            {car.description && (
                                <div className="glass rounded-2xl p-6">
                                    <h2 className="text-xl font-bold text-white mb-4">Giới thiệu chi tiết</h2>
                    ...
                    {activeTab === 'exterior' && car.exterior && (
                        <div className="space-y-6">
                            {car.exterior.description && (
                                <div className="glass rounded-2xl p-6">
                                    <h2 className="text-xl font-bold text-white mb-4">🚘 Ngoại thất</h2>
                                    <p className="text-text-secondary leading-relaxed whitespace-pre-wrap">
                                        {car.exterior.description}
                                    </p>
                                </div>
                    ...
                    {activeTab === 'interior' && car.interior && (
                        <div className="space-y-6">
                            {car.interior.description && (
                                <div className="glass rounded-2xl p-6">
                                    <h2 className="text-xl font-bold text-white mb-4">🛋️ Nội thất</h2>
                                    <p className="text-text-secondary leading-relaxed whitespace-pre-wrap">
                                        {car.interior.description}
                                    </p>
                                </div>
                    ...
                    {activeTab === 'colors' && car.colorOptions && car.colorOptions.length > 0 && (
                        <div className="space-y-6">
                            <div className="glass rounded-2xl p-6">
                                <h2 className="text-xl font-bold text-white mb-4">🎨 Tùy chọn màu sắc</h2>
                                <p className="text-text-secondary mb-6">
                                    {car.name} có {car.colorOptions.length} tùy chọn màu. Chọn màu bên dưới để xem hình ảnh minh họa.
                                </p>

                                {/* Color Swatches */}
                                <div className="flex flex-wrap gap-3 mb-6">
                                    {car.colorOptions.map((color: { name: string; hexCode: string; image?: string }, idx: number) => (
                                        <button
                                            key={idx}
                                        ...
                                        >
                                            ...
                                            />
                                            <span className={selectedColorIndex === idx ? 'text-primary font-bold' : 'text-text-secondary'}>
                                                {color.name}
                                            </span>
                                        </button>
                                    ))}
                                ...
                                {car.colorOptions[selectedColorIndex]?.image ? (
                                    ...
                                    <div
                                        ...
                                        style={{ backgroundColor: car.colorOptions[selectedColorIndex]?.hexCode || '#333' }}
                                    >
                    ...
                    {activeTab === 'specs' && car.specs && (
                        <div className="glass rounded-2xl overflow-hidden">
                            <h2 className="text-xl font-bold text-white p-6 border-b border-border">⚙️ Thông số kỹ thuật</h2>
                            <table className="w-full">
                                <tbody className="divide-y divide-border">
                                    {car.specs.engine && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Động cơ</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.engine}</td>
                                        </tr>
                                    )}
                                    {car.specs.power && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Công suất</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.power}</td>
                                        </tr>
                                    )}
                                    {car.specs.torque && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Mô-men xoắn</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.torque}</td>
                                        </tr>
                                    )}
                                    {car.specs.acceleration && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Tăng tốc 0-100km/h</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.acceleration}</td>
                                        </tr>
                                    )}
                                    {car.specs.topSpeed && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Tốc độ tối đa</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.topSpeed}</td>
                                        </tr>
                                    )}
                                    {car.specs.range && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Quãng đường</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.range}</td>
                                        </tr>
                                    )}
                                    {car.specs.fuelConsumption && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Tiêu hao nhiên liệu</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.fuelConsumption}</td>
                                        </tr>
                                    )}
                                    {car.specs.seats && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Số chỗ ngồi</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.seats} chỗ</td>
                                        </tr>
                                    )}
                                    {car.specs.dimensions && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Kích thước (DxRxC)</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.dimensions}</td>
                                        </tr>
                                    )}
                                    {car.specs.weight && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Trọng lượng</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.weight}</td>
                                        </tr>
                                    )}
                                    {car.specs.transmission && (
                                        <tr>
                                            <td className="px-6 py-4 text-text-secondary">Hộp số</td>
                                            <td className="px-6 py-4 text-white font-medium">{car.specs.transmission}</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                ...
                {relatedPosts.length > 0 && (
                    <div className="mt-12">
                        <h2 className="text-xl font-bold text-white mb-4">Tin tức về xe này</h2>
