...
export const postsAPI = {
    getAll: (params?: { category?: string; search?: string; page?: number; limit?: number }) =>
        api.get<{ success: boolean; data: Post[]; pagination: { page: number; total: number; pages: number } }>('/posts', { params }),

    getBySlug: (slug: string) =>
        api.get<{ success: boolean; data: Post }>(`/posts/${slug}`),
};

export const carsAPI = {
    getAll: (params?: { brand?: string; carType?: string; minPrice?: number; maxPrice?: number; featured?: boolean; search?: string; page?: number; limit?: number }) =>
        api.get<{ success: boolean; data: Car[]; pagination: { page: number; total: number; pages: number } }>('/cars', { params }),

    getBySlug: (slug: string) =>
        api.get<{ success: boolean; data: Car }>(`/cars/${slug}`),
...
export const notificationsAPI = {
    contact: (data: { type: 'contact_car' | 'contact_post'; refId: string; refTitle: string; refThumbnail?: string }) =>
